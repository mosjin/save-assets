...
在写下任何**「可能没做」「可能不可用」「还没建好」「最可能的解释是……」**这类关于项目状态的否定或猜测之前，必须先检索已保存的资产：

```bash
grep -rn "<关键词>" {DOCS_DIR}/          # CHANGELOG / TECH_LOG / VERIFIED_FACTS 等
gh issue view {N} --comments             # 该 issue 自己的历史
```

查不到才可以说"不清楚"。**永远不要**从"我记得当时还在做 X"推出"所以现在 X 还没做好"——记忆里的进行时状态没有日期，判不出它有没有过期。
...
- 之前记录过的事实，**状态**变了吗？（阻塞解除、能力验证通过、假设被推翻）
...
| 持久事实 | 架构、偏好、约束 | 记忆文件 |
| 已验证能力状态 | 经证据确认可用 | `docs/VERIFIED_FACTS.md` |
...
**判断失误** —— 当本次会话的发现推翻了之前的某个断言时，改用这个子模板：

```markdown
- **错判：** {当时断言了什么}
  - **依据：** {当时依据的是什么}
  - **错在哪：** {那个依据本身的问题 —— 过期？没查证？以偏概全？}
  - **下次先查：** {具体的检索动作}
```
...
## 第五步之三：更新 VERIFIED_FACTS.md（已验证能力状态）

文件：`docs/VERIFIED_FACTS.md`

用于记录**会过期**的事实 —— "某能力已验证可用"、"某权限已授予"、"某备份能还原" —— 区别于第一步的"持久事实"（架构、约束、偏好，基本不过期）。

每条记录必须包含以下五个字段：

```markdown
### {能力，写成陈述句 —— "X 可用"而不是"正在做 X"}
- **事实**：{陈述句}
- **验证日期**：{YYYY-MM-DD 绝对日期}
- **证据**：{可复核的：日志原文、命令输出、截图路径、issue 链接。"我记得测过"不算}
- **谁验的**：{用户在真机上验证 / CI / 我读日志验证 —— 份量不同}
- **失效条件**：{什么情况下要重新验证}
```

**规则：** 台账里的事实只能被新证据推翻，不能被推理推翻——读代码得出的结论，压不过用户在真机上的一次成功操作。
...
扫一遍记忆文件里的进行时表述（"正在"/"待"/"等待"/"TODO：等某人"），逐条问"这件事完成了吗"：
- 完成了 → 改写成带**绝对日期**的事实陈述句，不要留原句
- 还没完成 → 保留，但补上**记录日期**，让下次能判断它有没有过期

**为什么重要：** 没有任何自动机制会在值到位时把这类记录翻篇。记忆文件每次会话自动加载，它的可见性天生压过埋在文档深处的正确记录——不主动作废，过期的"正在做"就会一直赢。
